...
public class NewAPI extends HttpServlet {
	...
	private static final long serialVersionUID = -3180706158715413007L;
...
//						}
						...
						Map uploadResult = CloudinaryUtil.getInstance().uploader().upload(uploadedFile,
								ObjectUtils.asMap("resource_type", "image", "folder", "news"));
						uploadedFile.delete();
				...
				model = mapper.readValue(jsonString, NewModel.class);
				...
				ApiResponse<NewModel> responseObj = new ApiResponse<>("Thêm topic thành công", "success", model);
	...
	protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (isMultipart) {
			...
			FileItemFactory factory = new DiskFileItemFactory();
			ServletFileUpload upload = new ServletFileUpload(factory);
			NewModel updateNew = new NewModel();
...
//						}
				...
				updateNew = mapper.readValue(jsonString, NewModel.class);
				...
				ApiResponse<NewModel> responseObj = new ApiResponse<>("Cập nhật thành công", "success", updateNew);
				mapper.writeValue(resp.getOutputStream(), responseObj);
