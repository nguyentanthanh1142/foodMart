...
<html>
<head>
<title>Chỉnh sửa chương trình khuyến mãi</title>
        ...
        Thêm chương trình khuyến mãi
       	...
       	Cập chương trình khuyến mãi
									...
									<div class="card-tools">
										<a class="btn btn-sm btn-danger" href="${PromtionURL }"><i
									...
									<div class="col-xs-12 col-md-6">
										<label class="col control-label no-padding-right">Loại
											...
											<select name="byPercent" id="byPercent" class="form-control">
												...
												<option value="false" <c:if test="${!model.byPercent}">selected</c:if>>Giảm giá theo vnđ</option>
											</select>
										</div>
									</div>
									<div class="col-xs-12 col-md-6">
										<label class="col control-label no-padding-right">Phạm vi</label>
										<div class="col-12">
											<select name="isProductPromotion" id="isProductPromotion" class="form-control">
												<option value="false">Tất cả</option>
												<option value="true" <c:if test="${model.isProductPromotion}">selected</c:if>>Sản phẩm cụ thể</option>
											</select>
										</div>
									</div>
								</div>
						...
						success : function(result) {
						     if (result.status === 'success') {
				                    Swal.fire({
				                        ...
				                        window.location.href = "${PromtionURL}?type=edit&id=" + encodeURIComponent(result.data.id);
				                    });
				                } else {
				                    Swal.fire({
				                        icon: 'error',
